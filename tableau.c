...
void tableau_generate(TABLEAU T_young)   {
    ...
    talbleau_minHeap_buildHeap(T_young);  //costruisco l'heap da array dato
...
void tableau_insertKey(TABLEAU T_young)  {
    ...
    *(T_young[idx]) = io_getInteger();   //posiziono la chiave nell'ultima posizione
    while(idx > 1 && *(T_young[talbleau_minHeap_padre(idx)]) > *(T_young[idx]))  {    //risalgo il sottoalbero per posizionare il valore nel posto giusto
        talbleau_minHeap_swap(T_young, idx, talbleau_minHeap_padre(idx));
...
int tableau_searchKey(TABLEAU T_young, int idx, int key)    {
    if(T_young[idx])   {   //se != NULL
        if(key >= *(T_young[idx]))  {   //controllo se la chiave è più grande del valore visitato nella Tableau
            int idx_ret = tableau_searchKey(T_young, talbleau_minHeap_sinistro(idx), key); //ricorsivamente scendo a sinistra
            ...
            return tableau_searchKey(T_young, talbleau_minHeap_destro(idx), key);  //ricorsivamente scendo a destra
        }
    }
...
void tableau_deleteKey(TABLEAU T_young) {
    ...
    if((idx = idx_p = tableau_searchKey(T_young, 1, io_getInteger())) != 0)   {  //se trovato, restituisce l'indice del valore
        ...
        if(*(T_young[*(T_young[0])]) < val_del)   {  //confronto il valore eliminato con il sostituito, utile al riordino in Heap
            while(idx > 1 && *(T_young[talbleau_minHeap_padre(idx)]) > *(T_young[idx]))  {    //riordino a salire dalla posizione attuale
                talbleau_minHeap_swap(T_young, idx, talbleau_minHeap_padre(idx));
                idx = talbleau_minHeap_padre(idx);
            }
        } else    {
            talbleau_minHeap_heapify(T_young, idx); //riordino a scendere dalla posizione attuale
        }
        tableau_print(T_young);    //stampa della Tableau aggiornata
...
int tableau_extractMin(TABLEAU T_young) {
    ...
    talbleau_minHeap_heapify(T_young, 1);   //riordino in Heap dalla radice
